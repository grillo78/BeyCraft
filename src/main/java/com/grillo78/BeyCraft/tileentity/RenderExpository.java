...
public class RenderExpository extends TileEntitySpecialRenderer<ExpositoryTileEntity>{
	...
	public void render(ExpositoryTileEntity te, double x, double y, double z, float partialTicks, int destroyStage,
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation("minecraft:textures/blocks/concrete_white.png"));
		...
		if(te.getStackInSlot(2).getCount()!=0) {
			...
			Minecraft.getMinecraft().getRenderItem().renderItem(te.getStackInSlot(2), ItemCameraTransforms.TransformType.FIXED);
		}
		if(te.getStackInSlot(1).getCount()!=0) {
			GlStateManager.translate(0, 0, ((ItemBeyDisk)te.getStackInSlot(1).getItem()).height);
		...
		if(te.getStackInSlot(0).getCount()!=0) {
			...
			Minecraft.getMinecraft().getRenderItem().renderItem(te.getStackInSlot(0), ItemCameraTransforms.TransformType.FIXED);
		}
		angle++;
