            ...
            if (this.getRotationSpeed() > 0 && (world.getBlockState(this.getPosition().down())
                ...
                if (!rotationStarted) {
                    ...
                    if (this.getLayer().getItem() instanceof ItemBeyLayerGT) {
                        setRotationSpeed(
                                getRotationSpeed() - ((ItemBeyDriver) getDriver().getItem()).getFriction() / (10 * (((ItemBeyDisc) getDisc().getItem()).getWeight() + ((ItemBeyGTWeight) ItemStack.read(getLayer().getTag().getCompound("weight")).getItem()).getWeight())));
                    } else {
                        setRotationSpeed(
                                getRotationSpeed() - 0.005F * ((ItemBeyDriver) getDriver().getItem()).getFriction() / (10 * ((ItemBeyDisc) getDisc().getItem()).getWeight()));
                ...
                if (getRadius() == 0 && new Random().nextInt(10) == 1) {
                    ...
                    if (new Random().nextInt(100) == 1) {
                        getLayer().getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                            attackEntityFrom(DamageSource.causeMobDamage((LivingEntity) entityIn), ((ItemBeyLayer) ((EntityBey) entityIn).getLayer().getItem()).getAttack() + ((ItemBeyLayer) getLayer().getItem()).getBurst() - ((ItemBeyLayer) getLayer().getItem()).getDefense());
                        });
                    } else {
                        getLayer().getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                            attackEntityFrom(DamageSource.GENERIC, ((ItemBeyLayer) ((EntityBey) entityIn).getLayer().getItem()).getAttack() + ((ItemBeyLayer) getLayer().getItem()).getBurst() - ((ItemBeyLayer) getLayer().getItem()).getDefense());
                        });
                    }
                } else {
                    getLayer().getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
