...
public class ItemCreator {
    ...
    public static void getItemsFromFolder() {
        ...
        try {
            new File(beycraftResourcePack.getPath() + "/assets/beycraft/lang").mkdir();
            FileWriter langFileWriter = new FileWriter(new File(beycraftResourcePack.getPath() + "/assets/beycraft/lang/en_us.json"));
            langFileWriter.write("{\n");
            for (int i = 0; i < itemsFiles.length; i++) {
                fileReader = new FileReader(itemsFiles[i]);
                properties.load(fileReader);
                if (i==itemsFiles.length-1) {
                    langFileWriter.write("\"item.beycraft."+itemsFiles[i].getName().replace(".properties", "")+"\": \""+properties.get("name")+"\"\n");
                }else{
                    langFileWriter.write("\"item.beycraft."+itemsFiles[i].getName().replace(".properties", "")+"\": \""+properties.get("name")+"\",\n");
                }
                if (properties.get("part").equals("layer")) {
                    new ItemBeyLayer(itemsFiles[i].getName().replace(".properties", ""),
