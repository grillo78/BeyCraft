		...
		stack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
			...
			if (stack.hasTag() && stack.getTag().contains("chip")) {
				Minecraft.getInstance().getItemRenderer().renderItem(
						ItemStack.read((CompoundNBT) stack.getTag().get("chip")), TransformType.FIRST_PERSON_LEFT_HAND,
			...
			if (stack.hasTag() && stack.getTag().contains("weight")) {
				...
				Minecraft.getInstance().getItemRenderer().renderItem(
						...
						TransformType.FIRST_PERSON_LEFT_HAND, combinedLightIn, OverlayTexture.NO_OVERLAY, matrixStack,
						buffer);
			}
			matrixStack.pop();
			matrixStack.push();
			if (stack.hasTag()&&stack.getTag().contains("isEntity")&&!stack.getTag().getBoolean("isEntity")) {
				...
				Minecraft.getInstance().getItemRenderer().renderItem(
						ItemStack.read((CompoundNBT) stack.getTag().get("disc")), TransformType.FIRST_PERSON_LEFT_HAND,
						combinedLightIn, OverlayTexture.NO_OVERLAY, matrixStack, buffer);
