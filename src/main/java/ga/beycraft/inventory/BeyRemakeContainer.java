...
public class BeyRemakeContainer extends Container {
     ...
     * @param id
     */
    public BeyRemakeContainer(ContainerType<?> type, int id, ItemStack stack, PlayerInventory playerInventory) {
        super(type, id);
        this.stack = stack;
        ...
        if(!BeyCraftRegistry.ITEMSCLEARWHEEL.isEmpty()){
            stack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                if(stack.hasTag()){
                    ...
                    h.insertItem(2, ItemStack.of(stack.getTag().getCompound("FusionWheel")),false);
                }
                this.addSlot(new SlotBeyDisk(h, 0, 10, 15));
                this.addSlot(new SlotBeyDriver(h, 1, 10, 35));
                this.addSlot(new SlotFusionWheel(h, 2, 38, 15));
            });
        }
    ...
    public void removed(PlayerEntity player) {
        ...
        if (player instanceof ServerPlayerEntity) {
            stack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                ...
                nbt.put("FusionWheel", h.getStackInSlot(2).save(new CompoundNBT()));
            });
        }
    }
