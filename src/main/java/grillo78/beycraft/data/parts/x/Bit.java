...
public class Bit extends Beypart {
    private float friction = 1;
    private float height = 0;
    private float speed = 1;
    private float radiusReduction = 1;

    public Bit(ResourceLocation id, CompoundTag compoundTag) {
    ...
    public Bit(ResourceLocation id, JsonObject jsonObject) {
        super(id, jsonObject);
    ...
    public CompoundTag toCompound() {
        ...
        compoundTag.putString("type", "bit");
    ...
    public Item getPartItem() {
        return ModItems.X_BIT.get();
    }

    @Override
    public ResourceLocation getModel() {
        return ResourceLocation.fromNamespaceAndPath(getId().getNamespace(), "beyparts/x/bits/" + getId().getPath() + ".obj");
    }
}
