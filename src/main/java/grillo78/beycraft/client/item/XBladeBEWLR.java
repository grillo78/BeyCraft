...
public class XBladeBEWLR extends BlockEntityWithoutLevelRenderer {
    public XBladeBEWLR() {
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        if (stack.has(ModDataComponents.X_RATCHET))
            Minecraft.getInstance().getItemRenderer().renderStatic(stack.get(ModDataComponents.X_RATCHET).getStack(), ItemDisplayContext.NONE, packedLight, packedOverlay, poseStack, buffer, null, 0);
        if (stack.has(ModDataComponents.X_BIT))
            Minecraft.getInstance().getItemRenderer().renderStatic(stack.get(ModDataComponents.X_BIT).getStack(), ItemDisplayContext.NONE, packedLight, packedOverlay, poseStack, buffer, null, 0);
        poseStack.popPose();
    }
}
